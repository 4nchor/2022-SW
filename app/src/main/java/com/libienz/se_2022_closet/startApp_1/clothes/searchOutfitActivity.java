                ...
                if (searchClothes_rb.isChecked()) {
                    ...
                    userRef.child(user.getUid()).child("Clothes").addListenerForSingleValueEvent(new ValueEventListener() {
                        ...
                        public void onDataChange(@NonNull DataSnapshot snapshot) {
                            ...
                            //----------- 다른 해결해야 하는 문제점 ------------
                            //태그가 저장될 때 태그들 사이에 공백문자가 계속해서 저장됩니다. (배열의 한칸을 먹고 있어요 저번 부터 발견했는데 다만 따로 생각하신 바가 있을까봐 그냥 둡니다)
                            //검색하는 동안 언제까지 입력한 태그를 저장할지 언제 배열을 dump할지 정해주셔야 할 것 같습니다. 사용자가 새로운 검색을 하고 싶을 때를 위해서요
                            //코디에도 완전 똑같은 코드 쓰시면 됩니다. 데베 긁어오는 부분 한부분만 고치면 되겠네요
